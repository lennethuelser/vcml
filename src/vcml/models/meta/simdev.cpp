...
void simdev::write_chkpt(u32 val) {
#ifdef HAVE_LIBCRIU
    checkpoint::chkpt::instance().checkpoint();
    waitc = 1;  //signal to program that dump finished
#else
    log_error("build without CRIU - checkpointing is not supported");
#endif
}
...
simdev::simdev(const sc_module_name& nm):
    ...
    chkpt.allow_write_only();
    chkpt.on_write(&simdev::write_chkpt);

    waitc.allow_read_write();
}
